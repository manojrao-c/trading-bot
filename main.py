...
A real trader never sells what they don't own, and never buys what they already hold.
This file enforces that discipline across all three markets.
...
signal_log = []  # recent signals for dashboard


# ── Helpers ───────────────────────────────────────────────────
def _has_position(broker, symbol: str) -> bool:
    """Return True if we currently hold an open position in this symbol."""
    return symbol in broker.positions and broker.positions[symbol].get("qty", 0) > 0


def _signal_is_actionable(sig: dict, broker, symbol: str) -> tuple[bool, str]:
    """
    Real-trader logic:
      BUY  → only if we do NOT already hold this symbol
      SELL → only if we DO hold this symbol
    ...
    if direction == "SELL" and not holding:
        return False, "No position held — SELL skipped (not a short-seller)"
    return True, ""
...
def scan_us():
    ...
    for symbol in cfg.US_WATCHLIST:
        try:
            ...
            # ── Real-trader check: do we own this before selling? ──
            ...
            try:
                ...
                # Alert ONLY after the order is confirmed
...
def scan_nse():
    ...
    # Check stop-losses / take-profits on existing positions first
    exits = nse_broker.check_stops()
    for ex in exits:
...
# ── CRYPTO scan ───────────────────────────────────────────────
def scan_crypto():
    """Crypto is 24/7 — always runs."""
    console.print("[bold yellow]₿  Scanning Crypto watchlist...[/]")

    # Check stop-losses / take-profits on existing positions first
    exits = crypto_broker.check_stops()
    for ex in exits:
        console.print(f"  [yellow]Auto-exit: {ex['symbol']} — {ex['reason']} @ {ex['price']}[/]")
    ...
    for symbol in cfg.CRYPTO_WATCHLIST:
        try:
            ...
            try:
                ...
                console.print(f"  [green]✓ Crypto order placed[/]")
            except Exception as e:
                console.print(f"  [red]Order failed: {e}[/]")
...
def run_scan():
    ...
    try:
        us_port = us_broker.portfolio_summary()
        nse_port = nse_broker.portfolio_summary()
        ...
        render_dashboard(
            ...
            crypto_summary={**crypto_risk.daily_summary(), "max_trades": cfg.MAX_TRADES_PER_DAY},
        )
    except Exception as e:
...
def main():
    ...
    alert.startup(["US Stocks (Alpaca Paper)", "NSE India (Paper)", "Crypto (Binance Testnet)"])

    if args.summary:
        us_port = us_broker.portfolio_summary()
        nse_port = nse_broker.portfolio_summary()
        crypto_port = crypto_broker.portfolio_summary()
        console.print_json(data={"US": us_port, "NSE": nse_port, "CRYPTO": crypto_port})
        return
